...
const ItemList = ({items}) => {   
    ...
    const load_prod = () => {
        const resultado = items.map(
            (p) => p.stock>=0 && (<Link to={'/tienda/item/'+p.id} key={'itemLink'+p.id}><Item key={'item_'+p.id} id={'product_'+p.id} title={p.title} price_range={p.price_range} pictureUrl={p.pictureUrl} s={p.stock} /></Link>)  
        )
        return resultado 
    }    
